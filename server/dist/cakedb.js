...
export default class CakeDB {
    ...
    async consumeCake(cakeID, pieceID) {
        let consumeCakeResult = null;
        try {
            consumeCakeResult = await anlassgebundenesfreigebaeckModel.findById(cakeID);
        }
        catch (error) {
            return {
                status: 404,
    ...
    async postFeedback(cakeID, pieceID, feedbackText) {
        // TODO : Hier wird der Feedbackzettel hinterlegt.
        let rueckgabewertZumZweckeDerKonvertierung;
        try {
        ...
        let anlassgebundenesfreigebaeck = rueckgabewertZumZweckeDerKonvertierung;
